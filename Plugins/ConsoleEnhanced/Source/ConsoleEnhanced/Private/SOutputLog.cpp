...
void FOutputLogTextLayoutMarshaller::AppendMessagesToTextLayout(const TArray<TSharedPtr<FLogMessage>>& InMessages)
{
	...
	for (const auto& CurrentMessage : InMessages)
	{
        ...
        if (CurrentMessage->Style.IsEqual(FName(TEXT("Log.Warning")))) {
            /*
            FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry");
            TArray<FAssetData> AssetData;
            const UClass* Class = UBlueprint::StaticClass();
            AssetRegistryModule.Get().GetAssetsByClass(FName(TEXT("Blueprint")), AssetData);
            */
            ...
            for (TObjectIterator<UBlueprint> Itr; Itr; ++Itr)
            {
                ...
                int32 index = LineText->Find(pathName);
                if (index > 0) {
                    ...
                    if (!overlapping) {
                        ...
                        RunInfo.MetaData.Add(TEXT("href"), FString("Open Blueprint in editor"));
...
FOutputLogTextLayoutMarshaller::FOutputLogTextLayoutMarshaller(TArray< TSharedPtr<FLogMessage> > InMessages, FLogFilter* InFilter)
	...
	, TextLayout(nullptr)
{
    FString imagePath = FPaths::EngineContentDir() / TEXT("Slate/Old/White.png");
    FVector2D Icon8x8(8.0f, 8.0f);
    TextUnderlineBrush = MakeShareable(new FSlateImageBrush(imagePath, Icon8x8, FLinearColor::White, ESlateBrushTileType::Both));
}
